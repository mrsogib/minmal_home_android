...
class WallpaperRotationReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val changed = WallpaperHelper.setRandomWallpaperFromFolder(context, folder)
        if (changed && prefs.isDynamicAccentEnabled()) {
            AccentColorHelper.extractAccentColor(context)?.let { prefs.setAccentColor(it) }
        }
    }
}
